...
namespace ContentFactory.Areas.Identity.Pages.Account
{
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (await _userManager.IsInRoleAsync(user.Result, "Администратор"))
                    {
                        _logger.LogInformation("User logged in as admin");
                        return RedirectToAction("Index", "Admin", returnUrl);
                    }
                    else
                    {
                        _logger.LogInformation("User logged in as user");
